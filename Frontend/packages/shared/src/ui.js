...
export class UI extends LitElement {
    ...
    return html`
      <div class="ui-host">
        ...
        <md-dialog id="session-dialog">
          ...
          <div slot="content">
            Share this code with other participants to join:
            <h2 id="dialog-session-code" class="dialog-code"></h2>
          </div>
          <div slot="actions">
            <md-text-button @click=${() => this.shadowRoot.querySelector('#session-dialog').close()}>
              Close
            </md-text-button>
          </div>
        </md-dialog>
          ...
          <div class="tab-panel explanations-panel">
            ...
            <div class="explanations-content">              
              ${this.explanations.length === 0 
                ? html`<div class="empty-state"><p>No explanations yet.</p></div>`
                : html`
  ...
  _addTestExplanation() {
    explanationManager.addExplanation("Test", "This is a test explanation.");
  }
  ...
  static styles = [
    ...
    css`
      ...
      /* NEU: Styling für den Dialog-Code */
      .dialog-code {
        ...
        text-align: center;
        margin-top: 8px;
        user-select: all; /* Ermöglicht einfaches Kopieren */
      }
    `
  ];
}