...
from .dependencies import get_simulation_manager, get_session_manager_instance
...
class MessageRouter:
    ...
    async def _process_client_message(self, message: UniversalMessage):
        ...
        try:
            # HINZUGEFÜGT: Vollständige Logik zur Nachrichtenverarbeitung
            if message.type == 'stt.transcription':
                ...
                if self._session_manager and message.client_id:
                    ...
                    if code:
                        response = UniversalMessage(type='session.created', payload={'code': code}, destination=message.client_id, origin=message.origin, client_id=message.client_id)
                    else:
                ...
                if self._session_manager and code_to_join and message.client_id:
                    ...
                    if success:
                        response = UniversalMessage(type='session.joined', payload={'code': code_to_join}, destination=message.client_id, origin=message.origin, client_id=message.client_id)
                    else:
                ...
                logger.info(f"STT module connected: {message.client_id}. Acknowledged.")
                # No response needed for init message
    ...
    # HINZUGEFÜGT: Fehlende Hilfsfunktionen
    ...
    def _create_error_message(self, origin_msg: UniversalMessage, err_type: ErrorTypes, text: str) -> UniversalMessage:
        return UniversalMessage(
            ...
            destination=origin_msg.client_id,
            origin='MessageRouter',
            client_id=origin_msg.client_id # Use original client_id for context
        )
